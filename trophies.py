...
def trophy_giver(player):
    ...
    if "Skye" in player.kills:
        if player.kills["Skye"] >= 10:
            trophies["Hero"]["Let the Skye Fall"] = trophy_list["Hero"]["Let the Skye Fall"]
    ...
    if player.hero == "Ozo" and player.deathTot >= 10:
        trophies["Hero"]["Throwzo"] = trophy_list["Hero"]["Throwzo"]
    ...
    if player.hero == "Ringo" and player.deathTot == 2:
        trophies["Hero"]["Double Down"] = trophy_list["Hero"]["Double Down"]
    ...
    if player.deathTot == 0 and player.result == 1:
        trophies["Match Goals"]["Here to Impress"] = trophy_list["Match Goals"]["Here to Impress"]
    ...
    if player.dealt > 50000:
        trophies["Match Goals"]["Da Damage Dealer"] = trophy_list["Match Goals"]["Da Damage Dealer"]
    ...
    if player.killTot >= 0:
        trophies["Match Goals"]["Killed Someone"] = trophy_list["Hero"]["Killed Someone"]
